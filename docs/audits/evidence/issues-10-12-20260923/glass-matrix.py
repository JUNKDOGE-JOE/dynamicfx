...
client=Client();project=(out/'Upstream Glass.aep').resolve().as_posix()
t=ast.parse((out.parent/'upstream-glass-20260924/ae-upstream.py').read_text(encoding='utf-8'));exec(compile(ast.Module(body=[n for n in t.body if isinstance(n,ast.FunctionDef)],type_ignores=[]),'demo-functions','exec'))
if sys.argv[1]=='prepare':
    call('matrix-save','app.project.save();return JSON.stringify("saved");')
    for depth in [8,16,32]:
        target=(out/f'matrix-{depth}.aep').resolve().as_posix()
        code='if(!app.project.file||!(decodeURI(app.project.file.name)==="Upstream Glass.aep"||/^matrix-/.test(app.project.file.name)))throw Error("Unexpected project");var c=app.project.itemByID(1);while(app.project.renderQueue.numItems)app.project.renderQueue.item(1).remove();app.project.bitsPerChannel='+str(depth)+';var rows=[];'
