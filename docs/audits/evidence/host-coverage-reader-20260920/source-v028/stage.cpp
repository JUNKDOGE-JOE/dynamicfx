...
extern "C" std::int32_t host_stage_bind(SPBasicSuite* basic, std::int32_t id,
    AEGP_EffectRefH effect, std::int32_t index, std::int32_t layer, std::int32_t stage,
    std::int32_t apply, std::int32_t* changed)
{
    if (!basic || !effect || !changed || !(stage == 0 || stage == -1 || stage == -2)) return A_Err_PARAMETER;
    *changed = 0;
    const AEGP_StreamSuite7* stream = nullptr;
    A_Err err = basic->AcquireSuite(kAEGPStreamSuite, kAEGPStreamSuiteVersion7,
        reinterpret_cast<const void**>(&stream));
    ...
    AEGP_StreamValue2 value{};
    AEGP_LayerParamStage before = 0;
    err = stream->AEGP_GetStreamLayerParamAndStageValue(id, ref, &value, &before);
    if (err) return err;
    struct DisposeValue {
        const AEGP_StreamSuite7* suite; AEGP_StreamValue2* value;
        ~DisposeValue() { suite->AEGP_DisposeStreamValue(value); }
    } dispose_value{stream, &value};
    if (value.val.layer_id == layer && before == stage) return A_Err_NONE;
    *changed = 1;
    if (!apply) return A_Err_NONE;
    value.val.layer_id = layer;
    err = stream->AEGP_SetStreamLayerParamAndStageValue(id, ref, &value, stage);
    if (err) return err;
    AEGP_LayerParamStage after = 0;
    err = stream->AEGP_GetStreamLayerParamStageValue(id, ref, &after);
    if (!err && after != stage) return A_Err_GENERIC;
    return err;
}
