...
for shift in [0,128]:
 ...
 edit('shift-'+str(shift),code);time.sleep(3)
 for depth in [8,16,32]:
  ref,rm=render('shift-'+str(shift),'reference',14,26,depth,.5,1)
  border=int(np.count_nonzero(ref[:,:128]))
  for kind,comp,owner in [('ordinary',1,13),('adjustment',30,44)]:
   a,m=render('shift-'+str(shift),kind,comp,owner,depth,.5,1);delta=int(np.count_nonzero(a!=ref))
   row={'case':'shift-'+str(shift),'kind':kind,'depth':depth,'pixels':480000,'mismatches':delta,'outside_sample_nonzero':border if shift else None,'reference':rm,'actual':m,'status':'PASS' if delta==0 and (shift==0 or border>0) else 'FAIL'}
   reports.append(row);(out/'shift-summary.json').write_text(json.dumps(reports,indent=2),encoding='utf-8');print(json.dumps(row),flush=True)
