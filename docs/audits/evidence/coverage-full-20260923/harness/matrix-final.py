...
def execute(name,code,guard=True):
    ...
    (out/('final-'+name+'.result.json')).write_text(json.dumps({'args':args,'result':result},ensure_ascii=False,indent=2),encoding='utf-8')
...
def render(case,kind,comp,owner,depth,seconds,ds):
    ...
    dest=(out/('final-'+name+'.png')).resolve()
    ...
    (out/('final-'+name+'.native.log')).write_text(raw,encoding='utf-8')
...
def check(case,depth,seconds=.5,ds=1,previous=None,allow_empty=False):
    ...
    for kind,comp,owner in [('ordinary',1,13),('adjustment',30,44)]:
        ...
        reports.append(row);(out/'final-core-summary.json').write_text(json.dumps(reports,indent=2),encoding='utf-8');print(json.dumps(row),flush=True)
