...
sys.path.insert(0,r'E:/Code/AePlugin_Dynamicfx/spike/host-outline');from mcp_client import Client
sys.path.insert(0,str(root/'scripts/m5'));from check_deep import Psd
client=Client();project=(out/'Release acceptance.aep').resolve().as_posix()
t=ast.parse((out.parent/'upstream-glass-20260924/ae-upstream.py').read_text(encoding='utf-8').replace('Upstream Glass.aep','Release acceptance.aep'));exec(compile(ast.Module(body=[n for n in t.body if isinstance(n,ast.FunctionDef)],type_ignores=[]),'demo-functions','exec'))
if sys.argv[1]=='prepare':
    call('matrix-save','app.project.save();return JSON.stringify("saved");')
    for depth in [8,16,32]:
        target=(out/f'matrix-{depth}.aep').resolve().as_posix()
        code='if(!app.project.file||!(decodeURI(app.project.file.name)==="Release acceptance.aep"||/^matrix-/.test(app.project.file.name)))throw Error("Unexpected project");var c=app.project.itemByID(1);while(app.project.renderQueue.numItems)app.project.renderQueue.item(1).remove();app.project.bitsPerChannel='+str(depth)+';var rows=[];'
        for res,ds in [('Full',1),('Half',2),('Quarter',4)]:
            for seconds in [0,1.5,3]:
    ...
    for depth in [8,16,32]:
        ...
        print(json.dumps({'depth':depth,'rendered':9}),flush=True)
