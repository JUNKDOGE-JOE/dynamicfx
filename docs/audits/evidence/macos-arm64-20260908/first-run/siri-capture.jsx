...
var c=comp("DFX_siri");if(state(c).token%4!==1)throw Error("Siri not ready");
app.project.bitsPerChannel=32;app.purge(PurgeTarget.ALL_CACHES);var r={state:state(c),frames:[]};
for(var i=0;i<3;i++){c.resolutionFactor=[1,1];c.saveFrameToPng(i,new File("/Users/junk_doge/Documents/DynamicFX/scripts/out/macos/ae2026"+"/siri_t"+i+".png"));r.frames.push(i);}
c.resolutionFactor=[2,2];c.saveFrameToPng(1,new File("/Users/junk_doge/Documents/DynamicFX/scripts/out/macos/ae2026"+"/siri_half.png"));
c.resolutionFactor=[1,1];c.time=1;c.openInViewer();c.layer("input").selected=true;
app.project.save(new File("/Users/junk_doge/Documents/DynamicFX/scripts/out/macos/ae2026"+"/siri-demo.aep"));return r;
})();var res={ok:true,data:data};}catch(e){var res={ok:false,error:String(e),line:e.line};}
var f=new File("/Users/junk_doge/Documents/DynamicFX/scripts/out/macos/ae2026/siri-capture.json");f.encoding='UTF-8';if(!f.open('w'))throw Error(f.error);f.write(encode(res));f.close();return encode(res);})();