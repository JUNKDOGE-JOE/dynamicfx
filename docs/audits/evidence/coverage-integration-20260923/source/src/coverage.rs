...
pub fn readiness(state: f64) -> Result<(), Diag> {
    match state {
        1.0 => Ok(()),
        2.0 => Err(Diag::CoverageUnsupported),
        _ => Err(Diag::CoverageUnavailable),
    }
}
...
mod tests {
    ...
    fn unready_unknown_and_nonfinite_states_never_authorize_coverage() {
        assert_eq!(readiness(1.0), Ok(()));
        assert_eq!(readiness(2.0), Err(Diag::CoverageUnsupported));
        for state in [0.0, 3.0, -1.0, 1.5, f64::NAN, f64::INFINITY] {
            assert_eq!(readiness(state), Err(Diag::CoverageUnavailable));
        }
    }
    #[test]
