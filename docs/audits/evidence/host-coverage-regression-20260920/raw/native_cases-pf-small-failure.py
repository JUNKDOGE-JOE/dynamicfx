...
for row in rows:
 ...
 try:
  ...
  if len(candidates)!=1:raise RuntimeError("Expected one PF coverage path")
  ...
  result={**data,"status":"PASS" if max(diff)<=1 else "FAIL","max_error_8bit":max(diff),"different_pixels":sum(v!=0 for v in diff),"pixels":len(diff),"mcp_seconds":elapsed,"reference":ref.name}
