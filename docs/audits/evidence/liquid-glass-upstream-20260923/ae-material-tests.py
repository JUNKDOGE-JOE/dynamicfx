...
call('material-preset',helpers+'for(var i=1;i<=c.numLayers;i++)c.layer(i).selected=false;var ps=c.selectedProperties;for(var i=ps.length-1;i>=0;i--)try{ps[i].selected=false;}catch(ignore){}card.selected=true;var e=card.property("ADBE Effect Parade").property(1);e.name="Liquid Glass Studio";e.selected=true;card.savePreset(new File('+json.dumps(preset)+'));return JSON.stringify({bytes:File('+json.dumps(preset)+').length});')
