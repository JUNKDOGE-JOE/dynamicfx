...
for name,mask in masks.items():
    cov=mask[:,:,None].repeat(4,axis=2).astype('float32');bg=np.ones_like(cov);expected=np.minimum(cpu_distance(mask),64)
    words=[]
    for depth in [8,16,32]:
        a=render('distance-'+name+'-'+str(depth),field,cov,bg,depth,[w,h]);packed=np.rint(a*255).astype('uint8');bits=packed[:,:,0].astype('uint32')|(packed[:,:,1].astype('uint32')<<8)|(packed[:,:,2].astype('uint32')<<16)|(packed[:,:,3].astype('uint32')<<24);d=bits.view('float32');error=float(np.max(np.abs(d-expected)));words.append(bits)
        check('distance '+name+' '+str(depth),error<.00001,maximum_error=error)
    check('packed depth identity '+name,np.array_equal(words[0],words[1]) and np.array_equal(words[0],words[2]))
old=root/'scripts/out/liquid-glass-20260923';coverage=np.fromfile(old/'coverage.f32',dtype='<f4').reshape(400,640,4);background=np.fromfile(old/'background.f32',dtype='<f4').reshape(400,640,4)
for ds in [1,2,4]:
    h,w=400//ds,640//ds;cov=coverage.reshape(h,ds,w,ds,4).mean(axis=(1,3));bg=background.reshape(h,ds,w,ds,4).mean(axis=(1,3))
    for depth in [8,16,32]:
        a=render(f'material-{depth}-{ds}',source,cov,bg,depth,[640,400]);visual=a*cov+bg*(1-cov);Image.fromarray(np.rint(np.clip(visual,0,1)*255).astype('uint8'),'RGBA').save(out/f'material-{depth}-{ds}.png');check(f'material {depth} {ds}',np.isfinite(a).all(),physical=[w,h])
off=source.replace('label:"Amount" min:0 max:1 default:1','label:"Amount" min:0 max:1 default:0')
a=render('amount-zero',off,coverage,background,32,[640,400]);check('amount zero exact input',np.array_equal(a,background))
a=render('partial-opacity',source,coverage*.37125,background,32,[640,400]);b=np.fromfile(out/'material-32-1.f32',dtype='<f4').reshape(a.shape);check('partial opacity keeps the same optical shape',np.array_equal(a,b),maximum_error=float(np.max(np.abs(a-b))))
extreme=source.replace('default:1.4','default:4').replace('default:70.710678','default:160').replace('label:"Dispersion" min:0 max:50 default:7','label:"Dispersion" min:0 max:50 default:50').replace('label:"Linear RGB Input" hint:bool default:0','label:"Linear RGB Input" hint:bool default:1')
a=render('extreme-linear',extreme,coverage,background*2,32,[640,400]);check('extreme linear HDR remains finite',np.isfinite(a).all())
...
check('filtered color CPU comparison',max(errors)<.00001,maximum_error=max(errors))
