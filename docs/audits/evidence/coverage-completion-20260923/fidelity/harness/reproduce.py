...
call('prepare','if(!app.project.file||!(/^(production-coverage|full|cold|before|after[0-9]*)\\.aep$/.test(app.project.file.name)))throw Error("Unexpected project");app.project.save();app.open(new File('+json.dumps(base)+'));for(var i=1;i<=app.project.numItems;i++){var c=app.project.item(i);if(!(c instanceof CompItem))continue;for(var j=1;j<=c.numLayers;j++){var fx=c.layer(j).property("ADBE Effect Parade");for(var k=1;k<=fx.numProperties;k++)if(fx.property(k).matchName==="DynamicFx Host Shape Probe"){fx.property(k).property(2).setValue(1);fx.property(k).property(6).setValue(0);}}}app.project.save(new File('+json.dumps(project)+'));return "prepared";')
time.sleep(3)
