...
for depth in (8,16,32):
    name=f'matrix-{depth}-self-masks';offset=log.stat().st_size
    code='(function(){'+guard+f'app.project.bitsPerChannel={depth};var c=app.project.itemByID(866),l=c.layer(1),e=l.property("ADBE Effect Parade").property(1);l.adjustmentLayer=true;e.property(2).setValue(14);e.property(5).setValue(e.property(5).value+1);return e.property(5).value;'+'})()'
    serial=int(execute(name+'-stage',code));deadline=time.monotonic()+18
    while time.monotonic()<deadline:
        raw=tail(offset)
        if f'BACKGROUND_COMPLETE comp=866 serial={serial}' in raw:break
        time.sleep(.5)
    else:raise RuntimeError('Stage request did not complete')
    dest=(out/(name+'.png')).resolve().as_posix()
    execute(name,'(function(){'+guard+'var c=app.project.itemByID(866);c.layer(1).property("ADBE Effect Parade").property(1).property(2).setValue(21);app.purge(PurgeTarget.ALL_CACHES);c.saveFrameToPng(0,new File('+json.dumps(dest)+'));return "rendered";})()')
    for _ in range(20):
    ...
    source=frames((out/f'matrix-{depth}-source-reference.native.log').read_text(encoding='utf-8'))[0]
    for idx in (1,2):
        item={'depth':depth,'stage':'masks','checkout_id':idx,'pixels':480000,'native_word_mismatches':int(np.count_nonzero(actual[idx]!=expected)),'same_as_background_input':bool(np.array_equal(actual[idx],actual[0])),'mask_changes_pixels':int(np.count_nonzero(source!=expected)),'status':'PASS' if np.array_equal(actual[idx],expected) else 'FAIL'}
        reports.append(item);print(item,flush=True)
(out/'self-pixel-matrix.json').write_text(json.dumps(reports,indent=2),encoding='utf-8')
execute('self-matrix-save','(function(){'+guard+'app.project.save();return "saved";})()')
