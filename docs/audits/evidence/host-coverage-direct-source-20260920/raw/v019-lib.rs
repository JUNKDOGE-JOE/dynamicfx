...
enum Params { Time, Mode, Run, RenderPaths, Request, Original, Carrier, MaskRaster, ExplicitSource }
...
impl AdobePluginGlobal for Global {
        ...
        _: &mut ae::Parameters<Params>) -> Result<(), Error>
    {
        if matches!(command, ae::Command::GlobalSetup) {
            ...
            log("probe=0.0.17 initialized; managed carrier is diagnostic-only");
        }
