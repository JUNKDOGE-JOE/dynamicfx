...
const MAX_REVOCATIONS: usize = 65536;

#[derive(Default)]
struct Revocations { values: HashSet<u64>, exhausted: bool }
fn revocations() -> &'static RwLock<Revocations> {
    static VALUES: OnceLock<RwLock<Revocations>> = OnceLock::new();
    VALUES.get_or_init(|| RwLock::new(Revocations::default()))
}
fn revoked(certificate: u64) -> bool {
    revocations().read().map(|state| state.exhausted || state.values.contains(&certificate)).unwrap_or(true)
}
fn revoke(certificate: u64) {
    if !valid_certificate(certificate) { return; }
    if let Ok(mut state) = revocations().write() {
        if state.values.len() >= MAX_REVOCATIONS { state.exhausted = true; }
        else { state.values.insert(certificate); }
    }
}

pub fn valid_certificate(value: u64) -> bool { (4..=MAX_CERTIFICATE).contains(&value) }

pub fn render_restore(resetup: bool, read_only_project: bool, render_engine: bool) -> bool {
    resetup && (read_only_project || render_engine)
}
...
impl Permit {
    ...
    pub fn certificate(&self) -> u64 {
        ...
        if valid_certificate(value) && !revoked(value) { value } else { 0 }
    }
    pub fn cache_key(&self) -> [u8; 16] {
        let mut bytes = *b"DFXCVG01\0\0\0\0\0\0\0\0";
        bytes[8..].copy_from_slice(&self.certificate().to_le_bytes());
        bytes
    }
    pub fn revoke(&self) { revoke(self.0.swap(0, Ordering::AcqRel) & !PROVISIONAL); }
    pub fn resetup(&self, render_only: bool) {
        let value = self.0.load(Ordering::Acquire);
        if !render_only { revoke(value & !PROVISIONAL); }
    ...
    pub fn grant(&self) -> u64 {
        ...
        loop {
            ...
            if valid_certificate(certificate) && !revoked(certificate) {
...
mod tests {
    ...
    fn restored_certificates_need_render_only_resetup_and_ui_copies_revoke() {
        ...
        render.resetup(true);assert_eq!(render.certificate(), certificate);
        ui.resetup(false);
        assert_eq!(ui.certificate(), 0);assert_eq!(render.certificate(), 0);
        assert_eq!(source.certificate(), 0);
        assert_ne!(source.grant(), certificate);
        assert_ne!(ui.grant(), certificate);
        render.revoke();assert_eq!(render.certificate(), 0);
    }

    #[test]
    fn prepared_headless_restore_survives_normal_resetup_but_never_grants_pending_data() {
        for (flag, engine) in [(true, false), (false, true)] {
            let certificate = Permit::default().grant();
            let restored = Permit::restored(certificate);
            restored.resetup(render_restore(true, flag, engine));
            assert_eq!(restored.certificate(), certificate);
            let pending = Permit::restored(0);
            pending.resetup(render_restore(true, flag, engine));
            assert_eq!(pending.certificate(), 0);
        }
        assert!(!render_restore(false, true, true));
        assert!(!render_restore(true, false, false));
    }

    #[test]
    fn host_cache_dependency_changes_when_a_different_clone_revokes_the_certificate() {
        let owner = Permit::default();let certificate = owner.grant();
        let render = Permit::restored(certificate);render.resetup(true);
        let before = render.cache_key();assert_eq!(before, owner.cache_key());
        let duplicate = Permit::restored(certificate);duplicate.resetup(false);
        assert_ne!(before, render.cache_key());assert_eq!(render.cache_key(), Permit::default().cache_key());
        owner.grant();assert_ne!(before, owner.cache_key());
    }
    #[test]
