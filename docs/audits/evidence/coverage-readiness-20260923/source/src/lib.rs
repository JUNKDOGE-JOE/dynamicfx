            ...
            Command::SmartPreRender { mut extra } => {
                ...
                match cb.checkout_layer(
                    ...
                    Ok(res) => {
                        ...
                        // resolve.
                        let (declared_logical, externals_early, coverage_key) = {
                            let mut local = self.lock().map_err(|_| Error::Generic)?;
                            resolve_transported_definition(plugin, &mut local);
                            let declared = local
                                .compiled
                            ...
                            let externals = local
                                ...
                                .unwrap_or_default();
                            let coverage_key = externals.iter().any(|source| matches!(source, ExternalSource::Coverage { .. }))
                                .then(|| local.coverage_permit.cache_key());
                            (declared, externals, coverage_key)
                        };
                        if let Some(key) = coverage_key { cb.guid_mix_in_ptr(&key)?; }
