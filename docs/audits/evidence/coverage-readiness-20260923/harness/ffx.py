...
setup='if(!app.project.file||app.project.file.name!=="after4.aep")throw Error("Unexpected project");var c=app.project.itemByID(30),source=null;for(var i=1;i<=c.numLayers;i++)if(c.layer(i).id===44)source=c.layer(i);if(!source)throw Error("Source missing");'+select+'source.selected=true;source.property("ADBE Effect Parade").property(1).selected=true;source.savePreset(new File('+json.dumps(preset)+'));for(var i=1;i<=app.project.numItems;i++){var comp=app.project.item(i);if(!(comp instanceof CompItem))continue;for(var j=1;j<=comp.numLayers;j++){var fx=comp.layer(j).property("ADBE Effect Parade");for(var k=1;k<=fx.numProperties;k++)if(fx.property(k).matchName==="DynamicFx Host Shape Probe")fx.property(k).property(2).setValue(1);}}for(var i=1;i<=c.numLayers;i++)if(c.layer(i).id===47)c.layer(i).enabled=false;app.project.save(new File('+json.dumps(project)+'));return JSON.stringify({presetBytes:File('+json.dumps(preset)+').length,sourceState:source.property("ADBE Effect Parade").property(1).property(427).value});'
...
reference=canvas((out/'after4-reference.native.log').read_text(encoding='utf-8'))
reports=[]
