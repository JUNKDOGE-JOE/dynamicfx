...
def execute(name,code):
    ...
    (out/('readiness-'+name+'.result.json')).write_text(json.dumps({'args':args,'result':result},ensure_ascii=False,indent=2),encoding='utf-8')
...
def render(name,comp,depth,index=0):
    ...
    dest=(out/('readiness-'+name+'.png')).resolve()
    ...
    (out/('readiness-'+name+'.native.log')).write_text(raw,encoding='utf-8')
...
for depth in map(int,sys.argv[1:] or ['32']):
    ...
    for kind,comp in [('production',1),('adjustment',30)]:
        ...
        (out/('readiness-matrix-'+kind+'-'+str(depth)+'.json')).write_text(json.dumps(report,indent=2),encoding='utf-8')
