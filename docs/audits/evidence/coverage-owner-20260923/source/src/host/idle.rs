                ...
                for effect_index in 0..effect_count {
                    ...
                    let call_result = catch_unwind(AssertUnwindSafe(|| {
                        if effects.installed_key_from_layer_effect(&effect_ref)? == target_key {
                            ...
                            coverage_requests.insert((layer_id, effect_index), general_reply.compiled.as_ref()
                                .is_some_and(|effect| effect.externals.iter().any(|source|
                                    matches!(source, crate::ExternalSource::Coverage { .. }))));
