...
pub type Requests = HashMap<(u32, i32), bool>;
...
struct Owner { layer: LayerHandle, id: u32, effects: Vec<(i32, bool)> }
...
impl State {
    pub fn needs_scan(&self, project: i32, comp_id: i32, requests: &Requests) -> bool {
        requests.values().any(|value| *value) || self.enabled.keys().any(|key| key.0 == project && key.1 == comp_id)
    }
    ...
    pub fn reconcile(&mut self, project: i32, comp_id: i32, comp: &CompHandle, item: &ItemHandle,
        ...
        for index in 0..count {
            ...
            for effect_index in 0..total {
                ...
                if Some(key) == helper {
                    ...
                    let enabled = requests.get(&(layer_id, effect_index)).ok_or(ae::Error::BadCallbackParameter)?;
                    own.push((effect_index, *enabled));
                }
            }
            if let Some(binding) = reader_binding {
                ...
                let valid = total == 1 && own.is_empty() && binding.stage == -2 && marked && reader_active &&
                    ...
                    !flags.intersects(LayerFlags::VIDEO_ACTIVE | LayerFlags::LAYER_IS_3D) &&
        ...
        for owner in &owners {
            ...
            let enabled = owner.effects.iter().any(|(_, value)| *value);
    ...
    fn bind_owner(&self, owner: &Owner, reader: &Reader, id: PluginId, basic: *const ae::sys::SPBasicSuite,
        ...
        for (index, enabled) in &owner.effects {
            let effect = Effect { suite: &effects, handle: effects.layer_effect_by_index(&owner.layer, id, *index)? };
        ...
        for (index, enabled) in &owner.effects {
            let effect = Effect { suite: &effects, handle: effects.layer_effect_by_index(&owner.layer, id, *index)? };
            changed |= write_state(basic, id, &effect.handle, if *enabled && !remove { 1.0 } else { 0.0 })?;
        }
        Ok(changed)
    }
