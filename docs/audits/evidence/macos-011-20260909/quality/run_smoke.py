...
def main():
    cases = [
        (f"{lang}-d{depth}-v2", OUT / "fixtures" / f"smoke-v2.{lang}",
    ...
    cases.append(("siri-reference-d32-v2", ROOT / "examples/siri-reference.glsl",
    ...
    for name, source, width, height, logical_width, logical_height, time, depth in cases:
        ...
        runs.append(log)
        (OUT / "smoke-executions-v2.json").write_text(json.dumps(runs, indent=2) + "\n")
    ...
    for depth in (8, 32):
        check(f"GLSL WGSL d{depth} exact pixels",
              np.array_equal(images[f"glsl-d{depth}-v2"], images[f"wgsl-d{depth}-v2"]))
    report = dict(
    ...
    (OUT / "smoke-summary-v2.json").write_text(json.dumps(report, indent=2) + "\n")
