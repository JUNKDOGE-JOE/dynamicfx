...
| Field | Value |
|---|---|
| Status date | 2026-08-21 (two field defects recorded on the `prism` sample, AE 2026: new TR-CACHE-001 interrupted-render cache poisoning; TR-BIND-002/#6 flicker confirmed) |
...
| Exact next action | **Two open field defects on 0.0.4 are now recorded; both fixes are runtime work to delegate. (1) [TR-CACHE-001](TEST_MATRIX.md#tr-cache-001--interrupted-render-poisons-the-frame-cache-field-defect) — interrupted-preview cache poisoning: in `src/lib.rs` `Command::SmartRender`, return `Err(Error::InterruptCancel)` when the input `checkout_layer_pixels(0)` is aborted instead of filling transparent black and returning `Ok` (which makes AE cache a frame missing that layer); keep `fill(0)` only for the true `Ok(None)` no-input case; add the interrupted-preview harness leg. (2) [TR-BIND-002](TEST_MATRIX.md#tr-bind-002--copied-instance-corrupts-slot-mapping-field-defect) / issue [#6](https://github.com/JUNKDOGE-JOE/dynamicfx/issues/6) — copy flicker (confirmed again on AE 2026 this session): author the harness leg (instance A migrated plan, instance B fresh, render both with per-slot distinguishable values, both compile orders), record `FAIL` on the 0.0.4 artifact, then fix the runtime so each instance's persisted plan governs its own stream reads and slot UI (or key the registry by plan identity), and re-run M2/M3 batteries. Before that surfaced, the position was: 0.0.4 is released — the ADR-0037 valid-range fix shipped 2026-08-19 as a pre-release ([TR-REL-004](TEST_MATRIX.md#tr-rel-004--004-release-verification); artifact `BFE1AB9F…`, host-verified on AE 2025 + 2026 by [TR-0037-001](TEST_MATRIX.md#tr-0037-001--pool-valid-range-float1-negative-int10)). Nothing is in flight. The recorded, unscheduled candidates remain: (a) making the build byte-reproducible (the release procedure works around it by never rebuilding after verification — see TR-REL-003/004), and (b) the growth-pool label limitation (needs the `AEGP_SetStreamName` undo-group requirement understood). Environment unchanged: the working directory is a clone of this repository as of 2026-08-17 with the private history in a sibling archive (`git --git-dir=<archive>/.git`, ADR-0036 §2); `Cargo.lock` is gitignored and was carried over by hand, so a *new* clone resolves fresh dependency versions rather than the ones the verified artifact was built from. |
...
**Open correctness defect on the released 0.0.4 (field, 2026-08-21) — [TR-CACHE-001](TEST_MATRIX.md#tr-cache-001--interrupted-render-poisons-the-frame-cache-field-defect):** on the user's `prism` sample (AE 2026), interrupting a preview (play, then move the CTI) leaves poisoned frames in the cache — the composited frame is missing one DynamicFx layer's whole contribution (layer-hidden decomposition matched each cached bad frame to "Layer 1 hidden" at mean |diff| = 0.00000). The frame persists on cache re-read and recovers only after `app.purge(ALL_CACHES)`; three clean Render-Queue passes are bit-identical (batch never interrupts, so exports are unaffected). Cause: in `src/lib.rs` `Command::SmartRender`, an aborted input `checkout_layer_pixels(0)` (`InterruptCancel`) is flattened to `checked_out = None` and takes the ADR-0030 §5 empty-input branch — it fills the output transparent black and returns `Ok(())`, so AE caches the layer as successfully-rendered black. Fix: return `Err(Error::InterruptCancel)` on an aborted checkout (AE discards the frame); keep `fill(0)` only for the true `Ok(None)` no-input case. Runtime fix + harness leg to delegate; public issue [#7](https://github.com/JUNKDOGE-JOE/dynamicfx/issues/7). Evidence: [`docs/audits/evidence/field-20260821-prism-sample/`](audits/evidence/field-20260821-prism-sample/README.md) (Finding A). Field workaround: purge after interrupting a preview, or export through the Render Queue / `aerender`.
