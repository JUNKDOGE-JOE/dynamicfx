...
| Field | Value |
|---|---|
| Status date | 2026-08-21 (two field defects recorded on the `prism` sample, AE 2026; TR-CACHE-001 interrupted-render cache poisoning **fixed at `cfccd5d` and host-verified PASS on the fix build**; TR-BIND-002/#6 flicker confirmed, still open) |
...
| Exact next action | **[TR-BIND-002](TEST_MATRIX.md#tr-bind-002--copied-instance-corrupts-slot-mapping-field-defect) / issue [#6](https://github.com/JUNKDOGE-JOE/dynamicfx/issues/6) — copy flicker — is now the one open field defect.** The fix is drafted as **[ADR-0038](adr/0038-registry-key-per-binding-plan.md) (`Proposed`, 2026-08-21)**: the process registry keys a per-instance artifact (its `BindingPlan`, and the `externals` layer-wiring derived from it) by source alone, so the last instance to compile overwrites another same-source instance's mapping. Two mechanisms are drafted — **B (recommended): key the registry by `(source, plan identity)`, `StateToken` unchanged; A (deferred): the registry value carries no per-instance state**. **Next: the user picks a mechanism → ADR-0038 Accepted → delegate the runtime implementation → author the harness leg** (instance A migrated plan, instance B fresh, render both with per-slot distinguishable values incl. a layer input, both compile orders; record `FAIL` on 0.0.4 then green on the fix) → re-run M2/M3 batteries on AE 2025 + 2026. **Done this session:** [TR-CACHE-001](TEST_MATRIX.md#tr-cache-001--interrupted-render-poisons-the-frame-cache-field-defect) / issue [#7](https://github.com/JUNKDOGE-JOE/dynamicfx/issues/7) (interrupted-preview cache poisoning) was fixed at `cfccd5d` and host-verified `PASS` on the fix build on AE 2026 (3 rounds, 0 dropped-layer frames vs pre-fix 7); `cargo test` 131 passed. Not yet released — 0.0.4 in the wild still has the bug; a re-release (0.0.5) that bundles the #6 fix too is the natural packaging step, sequenced after #6. Still-open unscheduled candidates: byte-reproducible build (TR-REL-003/004) and the growth-pool label limitation. Environment note unchanged: the working directory is a clone of this repository as of 2026-08-17 with private history in a sibling archive (ADR-0036 §2); `Cargo.lock` is gitignored and carried by hand, so a new clone resolves fresh dependency versions. Before that surfaced, the position was: 0.0.4 is released — the ADR-0037 valid-range fix shipped 2026-08-19 as a pre-release ([TR-REL-004](TEST_MATRIX.md#tr-rel-004--004-release-verification); artifact `BFE1AB9F…`, host-verified on AE 2025 + 2026 by [TR-0037-001](TEST_MATRIX.md#tr-0037-001--pool-valid-range-float1-negative-int10)). Nothing is in flight. The recorded, unscheduled candidates remain: (a) making the build byte-reproducible (the release procedure works around it by never rebuilding after verification — see TR-REL-003/004), and (b) the growth-pool label limitation (needs the `AEGP_SetStreamName` undo-group requirement understood). Environment unchanged: the working directory is a clone of this repository as of 2026-08-17 with the private history in a sibling archive (`git --git-dir=<archive>/.git`, ADR-0036 §2); `Cargo.lock` is gitignored and was carried over by hand, so a *new* clone resolves fresh dependency versions rather than the ones the verified artifact was built from. |
