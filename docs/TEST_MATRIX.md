...
- Editor-flavor legs to run on the production `editor` build (both years, after the JSX index re-pin sweep): apply/expand/draw at panel widths incl. a dragged column; stage-2 gestures landing in the correct ADR-0033 rows with undo and keyframes; editor-on ↔ editor-off project interchange; reopen 13/13; M2 + M3 batteries + aerender. Per ADR-0042 verification obligations.
