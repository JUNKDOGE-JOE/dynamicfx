...
- Related: [ADR-0005](adr/0005-stable-parameter-ids.md), [ADR-0013](adr/0013-paramid-grammar-and-pools.md), [ADR-0016](adr/0016-sequence-schema-v1.md), [TR-M3-001](#tr-m3-001--persistence-and-render-clone); public issue [#6](https://github.com/JUNKDOGE-JOE/dynamicfx/issues/6). **Fix proposed in [ADR-0038](adr/0038-registry-key-per-binding-plan.md)** (`Proposed` 2026-08-21; two mechanisms, B recommended — key the registry by `(source, plan identity)`; awaiting the user's choice, then implementation + this harness leg).
