...
impl State {
    pub fn new(id: ae::aegp::PluginId, basic: *mut ae::sys::SPBasicSuite, alive: Arc<AtomicBool>) -> Self {
        Self { id, basic: basic as usize, thread: std::thread::current().id(), alive,
            ...
            carrier: super::carrier::State::default(), reader: super::reader::State::default() }
    }
}
...
fn tick(state: &mut State) -> Result<(), ae::Error> {
    if cfg!(feature = "preset-seed") { return seed_tick(state); }
    ...
    for project_index in 0..projects.num_projects()? {
        ...
        while let Some(item) = current {
            ...
            let manage_readers = items.item_name(&item, state.id)?.starts_with("HS_reader_");
            ...
            for layer_index in 0..layers.comp_num_layers(&comp)? {
                ...
                if manage_carriers {
                    state.carrier.reconcile((project_index as i32, item_id, layers.layer_id(&layer)?),
                        state.id, state.basic as *mut _, &layer, carrier_enabled)?;
                }
            }
            if manage_readers && state.reader.reconcile(project_index as i32, item_id, &comp, &item,
                state.id, state.basic as *mut _, target)? { return Ok(()); }
        }
    }
    Ok(())
}

fn seed_tick(state: &mut State) -> Result<(), ae::Error> {
    ...
    for project_index in 0..projects.num_projects()? {
        ...
        while let Some(item) = current {
            ...
            if items.item_type(&item)? != ae::aegp::ItemType::Comp ||
                !items.item_name(&item, state.id)?.starts_with("SDK_seed_") { continue; }
            let Some(comp) = comps.comp_from_item(&item)? else { continue };
            for index in 0..layers.comp_num_layers(&comp)? {
                ...
                for index in 0..effects.layer_num_effects(&layer)? {
                    budget = budget.checked_sub(1).ok_or(ae::Error::BadCallbackParameter)?;
                    let effect = EffectGuard { suite: &effects,
                        handle: effects.layer_effect_by_index(&layer, state.id, index)? };
                    let key = effects.installed_key_from_layer_effect(&effect.handle)?;
                    if effects.effect_match_name(key)? != "DynamicFx Host Shape Probe" { continue; }
                    for (index, stage) in [(1, -2), (2, -1)] {
                        let stream = streams.new_effect_stream_by_index(&effect.handle, state.id, index)?;
                        let value = streams.new_stream_value(&stream, state.id, ae::aegp::TimeMode::LayerTime,
                            ae::Time { value: 0, scale: 1 }, true)?;
                        if let ae::aegp::StreamValue::LayerId(target) = value {
                            if target <= 0 { continue; }
                            let changed = unsafe { super::stage::bind(state.basic as *mut _, state.id,
                                effect.handle.as_ptr(), index, target, stage, true) }.map_err(|e| {
                                    super::log(&format!("SEED_FAILED {e}")); ae::Error::Generic
                                })?;
                            if changed { super::log(&format!("SEED_BOUND index={index} target={target} stage={stage}")); }
                        }
                    }
                }
            }
